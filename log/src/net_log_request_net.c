...
void net_watcher_event_cb(void * ctx, int fd, uint8_t do_read, uint8_t do_write) {
    ...
    if (rc != CURLM_OK) {
        ...
        net_log_request_complete(schedule, request, net_log_request_complete_cancel);
        return;
    }
...
static void net_log_request_manage_check_multi_info(net_log_schedule_t schedule, net_log_request_manage_t mgr) {
    ...
    while ((msg = curl_multi_info_read(mgr->m_multi_handle, &msgs_left))) {
        ...
        case CURLMSG_DONE: {
            switch(res) {
            case CURLE_OK:
                net_log_request_complete(schedule, request, net_log_request_complete_done);
                break;
            case CURLE_OPERATION_TIMEDOUT:
                net_log_request_complete(schedule, request, net_log_request_complete_timeout);
                break;
            default:
                CPE_ERROR(
...
void net_log_request_manage_do_timeout(net_timer_t timer, void * ctx) {
    ...
    if (schedule->m_debug >= 2) {
        CPE_INFO(schedule->m_em, "log: schedule: net: do_timieout: curl_multi_socket_action(timeout)");
    }

    rc = curl_multi_socket_action(mgr->m_multi_handle, CURL_SOCKET_TIMEOUT, 0, &mgr->m_still_running);
    if (rc != CURLM_OK) {
        CPE_ERROR(schedule->m_em, "log: schedule: net: do_timeout: curl_multi_socket_action return error %d!", rc);
    }
