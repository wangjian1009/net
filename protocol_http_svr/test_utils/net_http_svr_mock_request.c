...
static void net_env_svr_mock_response_op_cb(void * ctx, test_net_tl_op_t op);

struct sfox_http_svr_mock_response_op {
...
{
    net_http_svr_mock_request_policy_t policy =
        mem_buffer_alloc(
    ...
    test_net_driver_t driver, net_http_svr_request_t request, net_http_svr_mock_request_policy_t policy)
{
    ...
    case net_http_svr_mock_request_type_response:
        if (policy->m_response.m_delay_ms == 0) {
            ...
            test_net_tl_op_t op = test_net_tl_op_create(
                ...
                sizeof(struct sfox_http_svr_mock_response_op),
                net_env_svr_mock_response_op_cb, driver, NULL);
            assert_true(op != NULL);

            struct sfox_http_svr_mock_response_op * response_op = test_net_tl_op_data(op);
            cpe_str_dup(
        ...
        break;
    case net_http_svr_mock_request_type_noop:
        break;
    }
}
