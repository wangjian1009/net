...
int net_endpoint_rbuf_supply(net_endpoint_t endpoint, uint32_t size) {
    ...
    if (endpoint->m_data_watcher_fun) {
        endpoint->m_data_watcher_fun(endpoint->m_data_watcher_ctx, endpoint, net_endpoint_data_r_supply, size);
    }
...
uint32_t net_endpoint_rbuf_size(net_endpoint_t endpoint) {
    ...
    if (endpoint->m_rb) {
        return ringbuffer_block_total_len(schedule->m_endpoint_buf, endpoint->m_rb);
    }
    else {
        return 0;
    }
...
int net_endpoint_rbuf_recv(net_endpoint_t endpoint, void * data, uint32_t * size) {
    ...
    *size = received;
    ...
    if (received) {
        if (endpoint->m_data_watcher_fun) {
            endpoint->m_data_watcher_fun(endpoint->m_data_watcher_ctx, endpoint, net_endpoint_data_r_consume, received);
        }
    }
