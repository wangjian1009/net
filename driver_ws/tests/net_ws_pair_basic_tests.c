...
static void net_ws_pair_text_msg_return_error(void **state) {
    net_ws_testenv_t env = *state;
    ...
    CPE_ERROR(env->m_em, "xxxxx: 111");
    assert_true(net_ws_endpoint_send_msg_text(cli_ep, "abcd") == 0);
    CPE_ERROR(env->m_em, "xxxxx: 111.1");
    test_net_driver_run(env->m_tdriver, 0);

    CPE_ERROR(env->m_em, "xxxxx: 222");
    assert_string_equal(
    ...
    CPE_ERROR(env->m_em, "xxxxx: 333");
    /* test_net_ws_endpoint_expect_text_msg(svr_ep, "efgh"); */
    /* assert_true(net_ws_endpoint_send_msg_text(cli_ep, "efgh") == 0); */
    /* test_net_driver_run(env->m_tdriver, 0); */
...
int net_ws_pair_basic_tests() {
	const struct CMUnitTest ws_basic_tests[] = {
		...
		cmocka_unit_test_setup_teardown(net_ws_pair_text_msg_return_error, setup, teardown),
	};
