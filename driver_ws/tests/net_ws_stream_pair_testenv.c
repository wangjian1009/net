...
net_endpoint_t net_ws_stream_pair_testenv_get_svr_ep(
    net_ws_stream_pair_testenv_t env, net_endpoint_t client_base_ep)
{
    net_ws_cli_endpoint_t cli_ep = net_endpoint_data(client_base_ep);
    net_endpoint_t underline = net_ws_cli_stream_endpoint_underline(client_base_ep);
    assert_true(underline);
    assert_true(net_endpoint_driver(underline) == net_driver_from_data(env->m_env->m_tdriver));

    net_endpoint_t svr_underline_base =  test_net_endpoint_linked_other(env->m_env->m_tdriver, underline);
