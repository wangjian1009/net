...
int net_ws_pair_testenv_acceptor_on_new_endpoint(void * ctx, net_endpoint_t endpoint) {
    net_endpoint_set_driver_debug(endpoint, 1);
    return 0;
}

net_ws_pair_testenv_t
net_ws_pair_testenv_create() {
    net_ws_pair_testenv_t env = mem_alloc(test_allocrator(), sizeof(struct net_ws_pair_testenv));
    env->m_env = net_ws_testenv_create();
    
    env->m_acceptor =
        net_ws_testenv_create_svr_acceptor(
            env->m_env, "1.2.3.4:5678",
            net_ws_pair_testenv_acceptor_on_new_endpoint, env);
...
net_endpoint_t net_ws_pair_testenv_get_svr_ep(
    net_ws_pair_testenv_t env, net_endpoint_t client_base_ep)
{
    net_ws_cli_endpoint_t cli_ep = net_endpoint_data(client_base_ep);
    ...
    net_ws_svr_underline_t svr_undline = net_endpoint_protocol_data(svr_underline_base);
    assert_true(svr_undline->m_ws_endpoint != NULL);
    
    net_endpoint_t svr_base_endpoint = net_endpoint_from_data(svr_undline->m_ws_endpoint);
