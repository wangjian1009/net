...
#define net_ws_cli_endpoint_handshake_field_is_set(__field) (endpoint->m_handshake.m_received_fields & (1u << __field))
#define net_ws_cli_endpoint_handshake_field_set(__field) (endpoint->m_handshake.m_received_fields |= (1u << __field))
const char * net_ws_cli_endpoint_handshake_field_str(net_ws_cli_endpoint_handshake_field_t field);

int net_ws_cli_endpoint_input_handshake_first_line(
    net_endpoint_t base_endpoint, net_ws_cli_endpoint_t endpoint, char * line, uint32_t line_sz)
{
...
int net_ws_cli_endpoint_input_handshake_header_line(
    net_endpoint_t base_endpoint, net_ws_cli_endpoint_t endpoint, char * line, uint32_t line_sz)
{
    net_ws_cli_protocol_t protocol = net_protocol_data(net_endpoint_protocol(base_endpoint));
    ...
    if (strcasecmp(name, "Upgrade") == 0) {
        ...
        net_ws_cli_endpoint_handshake_field_set(net_ws_cli_endpoint_handshake_field_upgrade);
    }
...
int net_ws_cli_endpoint_input_handshake_last_line(
    net_endpoint_t base_endpoint, net_ws_cli_endpoint_t endpoint)
{
    net_ws_cli_protocol_t protocol = net_protocol_data(net_endpoint_protocol(base_endpoint));

    /*检查必须的字段 */
    net_ws_cli_endpoint_handshake_field_t check_fields[] = {
    ...
    for(i = 0; i < CPE_ARRAY_SIZE(check_fields); ++i) {
        if (!net_ws_cli_endpoint_handshake_field_is_set(i)) {
            CPE_ERROR(
                protocol->m_em, "net: ws: %s: handshake: field %s not set!",
                net_endpoint_dump(net_ws_cli_protocol_tmp_buffer(protocol), base_endpoint),
                net_ws_cli_endpoint_handshake_field_str((net_ws_cli_endpoint_handshake_field_t)i));
            return -1;
        }
    }
...
int net_ws_cli_endpoint_input_handshake(net_endpoint_t base_endpoint, net_ws_cli_endpoint_t endpoint) {
    ...
    while(endpoint->m_state == net_ws_cli_endpoint_state_handshake) {
        ...
        if (line[0] == 0) {
            ...
            if (net_ws_cli_endpoint_input_handshake_last_line(base_endpoint, endpoint) != 0) return -1;
            endpoint->m_state = net_ws_cli_endpoint_state_streaming;
        }
        else {
            if (net_endpoint_protocol_debug(base_endpoint) >= 2) {
                CPE_INFO(
                    protocol->m_em, "net: ws: %s: handshake << %s",
                    net_endpoint_dump(net_ws_cli_protocol_tmp_buffer(protocol), base_endpoint), line);
            }

            switch (endpoint->m_handshake.m_state) {
            case net_ws_cli_endpoint_handshake_first_line:
                if (net_ws_cli_endpoint_input_handshake_first_line(
                        base_endpoint, endpoint, line, line_size - 2) != 0) return -1;
                endpoint->m_handshake.m_state = net_ws_cli_endpoint_handshake_header_line;
                break;
            case net_ws_cli_endpoint_handshake_header_line:
                if (net_ws_cli_endpoint_input_handshake_header_line(
                        base_endpoint, endpoint, line, line_size - 2) != 0) return -1;
            }
        }
...
const char * net_ws_cli_endpoint_handshake_field_str(net_ws_cli_endpoint_handshake_field_t field) {
    switch(field) {
    case net_ws_cli_endpoint_handshake_field_upgrade:
        return "upgrade";
    case net_ws_cli_endpoint_handshake_field_connection:
        return "connection";
    case net_ws_cli_endpoint_handshake_field_accept:
        return "accept";
    }
}
