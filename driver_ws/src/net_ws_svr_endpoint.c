...
int net_ws_svr_endpoint_update(net_endpoint_t base_endpoint) {
    ...
    case net_endpoint_state_established:
        if (!net_endpoint_buf_is_empty(base_endpoint, net_ep_buf_write)) { /*有数据等待写入 */
            if (net_ws_svr_underline_write(endpoint->m_underline, base_endpoint, net_ep_buf_write) != 0) return -1;
...
int net_ws_svr_endpoint_set_no_delay(net_endpoint_t base_endpoint, uint8_t no_delay) {
    net_ws_svr_driver_t driver = net_driver_data(net_endpoint_driver(base_endpoint));
    net_ws_svr_endpoint_t endpoint = net_endpoint_data(base_endpoint);
    ...
    if (endpoint->m_underline == NULL) {
        CPE_ERROR(
            driver->m_em, "net: ws: %s: set no delay: no underline!",
            net_endpoint_dump(net_ws_svr_driver_tmp_buffer(driver), base_endpoint));
        return -1;
    }
...
int net_ws_svr_endpoint_get_mss(net_endpoint_t base_endpoint, uint32_t * mss) {
    net_ws_svr_driver_t driver = net_driver_data(net_endpoint_driver(base_endpoint));
    net_ws_svr_endpoint_t endpoint = net_endpoint_data(base_endpoint);
...
net_endpoint_t
net_ws_svr_endpoint_create(net_ws_svr_driver_t driver, net_protocol_t protocol) {
    ...
    net_ws_svr_underline_t underline = net_endpoint_protocol_data(endpoint->m_underline);
...
net_endpoint_t
net_ws_svr_endpoint_underline(net_endpoint_t base_endpoint) {
    net_schedule_t schedule = net_endpoint_schedule(base_endpoint);

    if (net_driver_endpoint_init_fun(net_endpoint_driver(base_endpoint)) != net_ws_svr_endpoint_init) {
        CPE_ERROR(
