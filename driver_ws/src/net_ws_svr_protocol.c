...
int net_ws_svr_protocol_init(net_protocol_t base_protocol) {
    net_ws_svr_protocol_t protocol = net_protocol_data(base_protocol);
    protocol->m_alloc = NULL;
    protocol->m_em = NULL;
    return 0;
}

void net_ws_svr_protocol_fini(net_protocol_t protocol) {
}
    ...
    mem_allocrator_t alloc, error_monitor_t em)
{
    ...
    net_protocol_t base_protocol =
        net_protocol_create(
            ...
            name,
            /*protocol*/
            sizeof(struct net_ws_svr_protocol),
            net_ws_svr_protocol_init,
            net_ws_svr_protocol_fini,
            /*endpoint*/
            sizeof(struct net_ws_svr_endpoint),
            net_ws_svr_endpoint_init,
            net_ws_svr_endpoint_fini,
            net_ws_svr_endpoint_input,
            net_ws_svr_endpoint_on_state_change,
            NULL);
...
net_ws_svr_protocol_t
net_ws_svr_protocol_find(net_schedule_t schedule, const char * addition_name) {
    char name[64];
    if (addition_name) {
        snprintf(name, sizeof(name), "ws-svr-%s", addition_name);
    }
    ...
    return protocol ? net_ws_svr_protocol_cast(protocol) : NULL;
}

net_ws_svr_protocol_t
net_ws_svr_protocol_cast(net_protocol_t base_protocol) {
    return net_protocol_init_fun(base_protocol) == net_ws_svr_protocol_init
