...
#include "net_ws_cli_driver_i.h"
#include "net_ws_cli_endpoint_i.h"
#include "net_ws_cli_underline_i.h"

static int net_ws_cli_driver_init(net_driver_t driver);
static void net_ws_cli_driver_fini(net_driver_t driver);
    ...
    mem_allocrator_t alloc, error_monitor_t em)
{
    ...
    net_driver_t base_driver =
        net_driver_create(
            ...
            name,
            /*driver*/
            sizeof(struct net_ws_cli_driver),
            net_ws_cli_driver_init,
            net_ws_cli_driver_fini,
            NULL,
            /*timer*/
            ...
            net_ws_cli_endpoint_connect,
            net_ws_cli_endpoint_update,
            net_ws_cli_endpoint_set_no_delay,
            net_ws_cli_endpoint_get_mss,
            /*dgram*/
            0, NULL, NULL, NULL,
            /*watcher*/
            0, NULL, NULL, NULL);

    net_ws_cli_driver_t ws_driver = net_driver_data(base_driver);

    ws_driver->m_alloc = alloc;
    ws_driver->m_em = em;
    ws_driver->m_underline_driver = underline_driver;
    ws_driver->m_underline_protocol =
        net_ws_cli_underline_protocol_create(schedule, name, ws_driver);
...
static int net_ws_cli_driver_init(net_driver_t base_driver) {
    net_ws_cli_driver_t driver = net_driver_data(base_driver);
    driver->m_alloc = NULL;
    driver->m_em = NULL;
...
static void net_ws_cli_driver_fini(net_driver_t base_driver) {
    net_ws_cli_driver_t driver = net_driver_data(base_driver);
