...
#define net_ws_svr_endpoint_handshake_field_is_set(__field) (endpoint->m_handshake.m_received_fields & (1u << __field))
#define net_ws_svr_endpoint_handshake_field_set(__field) (endpoint->m_handshake.m_received_fields |= (1u << __field))
const char * net_ws_svr_endpoint_handshake_field_str(net_ws_svr_endpoint_handshake_field_t field);
    ...
    net_endpoint_t base_endpoint, net_ws_svr_endpoint_t endpoint, char * line, uint32_t line_sz)
{
    ...
    if (net_ws_svr_endpoint_set_path(endpoint, path) != 0) {
        CPE_ERROR(
    ...
    net_endpoint_t base_endpoint, net_ws_svr_endpoint_t endpoint, char * line, uint32_t line_sz)
{
    ...
    if (strcasecmp(name, "Host") == 0) {
        ...
        if (net_ws_svr_endpoint_set_host(endpoint, host) != 0) {
            CPE_ERROR(
        ...
        net_ws_svr_endpoint_handshake_field_set(net_ws_svr_endpoint_handshake_field_upgrade);
    }
        ...
        net_ws_svr_endpoint_handshake_field_set(net_ws_svr_endpoint_handshake_field_connection);
    }
        ...
        if (cpe_sha1_starts(&ctx) != 0
            ...
            || cpe_sha1_finish(&ctx, &endpoint->m_handshake.m_accept_key) != 0)
        {
        }
        cpe_sha1_fini(&ctx);
        net_ws_svr_endpoint_handshake_field_set(net_ws_svr_endpoint_handshake_field_key);
    }
    ...
    net_endpoint_t base_endpoint, net_ws_svr_endpoint_t endpoint)
{
    ...
    /*检查必须的字段 */
    net_ws_svr_endpoint_handshake_field_t check_fields[] = {
    ...
    for(i = 0; i < CPE_ARRAY_SIZE(check_fields); ++i) {
        if (!net_ws_svr_endpoint_handshake_field_is_set(i)) {
            CPE_ERROR(
                protocol->m_em, "net: ws: %s: handshake: field %s not set!",
                net_endpoint_dump(net_ws_svr_protocol_tmp_buffer(protocol), base_endpoint),
                net_ws_svr_endpoint_handshake_field_str((net_ws_svr_endpoint_handshake_field_t)i));
            return -1;
        }
    }
    ...
    cpe_base64_encode_from_buf(
        (write_stream_t)&ws,
        endpoint->m_handshake.m_accept_key.digest,
        sizeof(endpoint->m_handshake.m_accept_key.digest));
...
const char * net_ws_svr_endpoint_handshake_field_str(net_ws_svr_endpoint_handshake_field_t field) {
    switch(field) {
    case net_ws_svr_endpoint_handshake_field_upgrade:
        return "upgrade";
    case net_ws_svr_endpoint_handshake_field_connection:
        return "connection";
    case net_ws_svr_endpoint_handshake_field_key:
        return "key";
    }
}
