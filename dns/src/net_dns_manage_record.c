...
int net_dns_manage_add_record(net_dns_manage_t manage, net_address_t hostname, net_address_t address) {
    assert(hostname);
        ...
        if (entry->m_expire_time_s < cur_time_s) {
            if (manage->m_debug) {
                CPE_INFO(
                    ...
                    (const char *)net_address_data(hostname), manage->m_cfg_ttl_s);
            }
