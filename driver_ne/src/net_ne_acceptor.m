...
// static void net_ne_acceptor_cb(EV_P_ ev_io *w, int revents);
    ...
    net_address_t address, uint32_t accept_queue_size)
{
    ...
    // if (acceptor->m_fd == -1) {
    //     CPE_ERROR(
    //         em, "ne: acceptor: socket call fail, errno=%d (%s)!",
    ...
    //     CPE_ERROR(
    //         em, "ne: acceptor: set sock reuseaddr fail, errno=%d (%s)!",
    //         cpe_sock_errno(), cpe_sock_errstr(cpe_sock_errno()));
    ...
    //     CPE_ERROR(
    //         em, "ne: acceptor: bind error, errno=%d (%s)",
    //         cpe_sock_errno(), cpe_sock_errstr(cpe_sock_errno()));
    //     cpe_sock_close(acceptor->m_fd);
    //     mem_free(alloc, acceptor);
    //     return NULL;
    ...
    // ev_io_init(&acceptor->m_watcher, net_ne_acceptor_cb, acceptor->m_fd, EV_READ);
    // ev_io_start(driver->m_ne_loop, &acceptor->m_watcher);

    TAILQ_INSERT_TAIL(&driver->m_acceptors, acceptor, m_next_for_driver);

    if (driver->m_debug || net_schedule_debug(schedule)) {
        CPE_INFO(
            driver->m_em, "ne: acceptor: listen at %s (accept-queue-size=%d)",
...
void net_ne_acceptor_free(net_ne_acceptor_t acceptor) {
    net_ne_driver_t driver = acceptor->m_driver;
    net_schedule_t schedule = net_driver_schedule(net_driver_from_data(driver));
    mem_allocrator_t alloc = net_schedule_allocrator(schedule);

    // ev_io_stop(driver->m_ne_loop, &acceptor->m_watcher);
...
// static void net_ne_acceptor_cb(EV_P_ ev_io *w, int revents) {
//     net_ne_acceptor_t acceptor = w->data;
//     net_ne_driver_t driver = acceptor->m_driver;
//     net_schedule_t schedule = net_driver_schedule(net_driver_from_data(driver));
...
//     if (new_fd == -1) {
//         CPE_ERROR(
//             em, "ne: acceptor: accept error, errno=%d (%s)",
//             cpe_sock_errno(), cpe_sock_errstr(cpe_sock_errno()));
//         return;
//     }
...
//         CPE_ERROR(
//             em, "ne: acceptor: set non-block fail, errno=%d (%s)",
//             cpe_sock_errno(), cpe_sock_errstr(cpe_sock_errno()));
...
//     if (net_ne_endpoint_update_local_address(endpoint) != 0
//         || net_ne_endpoint_update_remote_address(endpoint) != 0)
//     {
//         net_endpoint_free(base_endpoint);
//         return;
//     }
