    ...
    net_address_t address, uint32_t accept_queue_size)
{
    ...
    net_dq_acceptor_t acceptor = mem_alloc(alloc, sizeof(struct net_dq_acceptor));
    if (acceptor == NULL) {
...
void net_dq_acceptor_free(net_dq_acceptor_t acceptor) {
    net_dq_driver_t driver = acceptor->m_driver;
    net_schedule_t schedule = net_driver_schedule(net_driver_from_data(driver));
    mem_allocrator_t alloc = net_schedule_allocrator(schedule);
