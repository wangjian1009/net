...
//static void net_dq_endpoint_rw_cb(EV_P_ ev_io *w, int revents);
//static void net_dq_endpoint_connect_cb(EV_P_ ev_io *w, int revents);
...
void net_dq_endpoint_fini(net_endpoint_t base_endpoint) {
    ...
    if (endpoint->m_fd != -1) {
        //ev_io_stop(driver->m_dq_loop, &endpoint->m_watcher);
...
int net_dq_endpoint_connect(net_endpoint_t base_endpoint) {
    ...
    if (cpe_connect(endpoint->m_fd, (struct sockaddr *)&addr, addr_len) != 0) {
        if (cpe_sock_errno() == EINPROGRESS || cpe_sock_errno() == EWOULDBLOCK) {
            ...
            ev_io_init(
                &endpoint->m_watcher,
                net_dq_endpoint_connect_cb, endpoint->m_fd,
                EV_READ | EV_WRITE);
            ev_io_start(driver->m_dq_loop, &endpoint->m_watcher);
            */
    ...
    net_dq_driver_t driver, net_endpoint_t base_endpoint, net_dq_endpoint_t endpoint)
{
    ...
    ev_io_init(
        &endpoint->m_watcher,
        net_dq_endpoint_rw_cb, endpoint->m_fd,
...
/*static void net_dq_endpoint_rw_cb(EV_P_ ev_io *w, int revents) {
    net_dq_endpoint_t endpoint = w->data;
    net_endpoint_t base_endpoint = net_endpoint_from_data(endpoint);
    net_dq_driver_t driver = net_driver_data(net_endpoint_driver(base_endpoint));
    net_schedule_t schedule = net_endpoint_schedule(base_endpoint);
    error_monitor_t em = net_schedule_em(schedule);
