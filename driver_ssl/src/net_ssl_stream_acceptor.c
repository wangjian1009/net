...
int net_ssl_stream_acceptor_on_new_endpoint(void * ctx, net_endpoint_t base_underline) {
    ...
    net_ssl_stream_acceptor_t acceptor = net_acceptor_data(base_acceptor);

    net_ssl_endpoint_t underline = net_ssl_endpoint_cast(base_underline);
    assert(underline);

    net_endpoint_t base_endpoint = NULL;
    net_ssl_stream_endpoint_t stream = NULL;
    ...
    if (base_endpoint == NULL) {
        CPE_ERROR(
            ...
            net_endpoint_dump(net_ssl_driver_tmp_buffer(driver), base_underline));
        goto INIT_ERROR;
    }
    stream = net_ssl_stream_endpoint_cast(base_endpoint);
    assert(stream);
...
int net_ssl_stream_acceptor_init(net_acceptor_t base_acceptor) {
    net_ssl_stream_driver_t driver = net_driver_data(net_acceptor_driver(base_acceptor));
    net_ssl_stream_acceptor_t acceptor = net_acceptor_data(base_acceptor);
...
void net_ssl_stream_acceptor_fini(net_acceptor_t base_acceptor) {
    net_ssl_stream_acceptor_t acceptor = net_acceptor_data(base_acceptor);
    net_ssl_stream_driver_t driver = net_driver_data(net_acceptor_driver(base_acceptor));
