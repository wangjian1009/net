...
static int net_nghttp2_service_init(net_protocol_t protocol);
static void net_nghttp2_service_fini(net_protocol_t protocol);

net_nghttp2_service_t
net_nghttp2_service_create(net_schedule_t schedule, const char * protocol_name) {
    net_protocol_t protocol =
        net_protocol_create(
            schedule,
            protocol_name,
            /*protocol*/
            sizeof(struct net_nghttp2_service),
            net_nghttp2_service_init,
            net_nghttp2_service_fini,
            /*endpoint*/
            sizeof(struct net_nghttp2_session),
            net_nghttp2_session_init,
            net_nghttp2_session_fini,
            net_nghttp2_session_input,
            NULL,
            NULL,
            NULL);
    if (protocol == NULL) {
    ...
    net_nghttp2_service_t service = net_protocol_data(protocol);
...
net_nghttp2_mount_point_t net_nghttp2_service_root(net_nghttp2_service_t service) {
    return service->m_root;
}

mem_buffer_t net_nghttp2_service_tmp_buffer(net_nghttp2_service_t service) {
    net_protocol_t protocol = net_nghttp2_service_to_protocol(service);
...
static int net_nghttp2_service_init(net_protocol_t protocol) {
    ...
    net_nghttp2_processor_t dft_processor = net_nghttp2_processor_create_dft(service);
    ...
    service->m_root = net_nghttp2_mount_point_create(
    ...
    if (service->m_root == NULL) {
        ...
        net_nghttp2_processor_free(dft_processor);
        return -1;
    }
...
static void net_nghttp2_service_fini(net_protocol_t protocol) {
    ...
    if (service->m_root) {
        net_nghttp2_mount_point_free(service->m_root);
        service->m_root = NULL;
    }
    
    while(!TAILQ_EMPTY(&service->m_processors)) {
        net_nghttp2_processor_free(TAILQ_FIRST(&service->m_processors));
    }
    
    while(!TAILQ_EMPTY(&service->m_free_requests)) {
        net_nghttp2_request_real_free(TAILQ_FIRST(&service->m_free_requests));
    }

    while(!TAILQ_EMPTY(&service->m_free_request_headers)) {
        net_nghttp2_request_header_real_free(TAILQ_FIRST(&service->m_free_request_headers));
    }

    while(!TAILQ_EMPTY(&service->m_free_responses)) {
        net_nghttp2_response_real_free(TAILQ_FIRST(&service->m_free_responses));
    }
    
    while (!TAILQ_EMPTY(&service->m_free_mount_points)) {
        net_nghttp2_mount_point_real_free(TAILQ_FIRST(&service->m_free_mount_points));
    }
