...
#include "net_nghttp2_response_i.h"

net_nghttp2_request_t net_nghttp2_request_create(net_nghttp2_connection_t connection) {
    net_nghttp2_service_t service = net_nghttp2_connection_service(connection);

    net_nghttp2_request_t request = TAILQ_FIRST(&service->m_free_requests);
    if (request) {
        TAILQ_REMOVE(&service->m_free_requests, request, m_next_for_connection);
    }
    else {
        request = mem_alloc(service->m_alloc, sizeof(struct net_nghttp2_request) + service->m_request_sz);
        if (request == NULL) {
            CPE_ERROR(
                service->m_em, "nghttp2: %s: request: alloc fail!",
                net_endpoint_dump(net_nghttp2_service_tmp_buffer(service), net_endpoint_from_data(connection)));
            return NULL;
        }
    }
    ...
    request->m_method = net_nghttp2_request_method_unknown;
    ...
    request->m_transfer_encoding = net_nghttp2_request_transfer_encoding_identity;
    request->m_keep_alive = -1;
    request->m_path = NULL;
    request->m_state = net_nghttp2_request_state_reading;
...
void net_nghttp2_request_free(net_nghttp2_request_t request) {
    net_nghttp2_connection_t connection = request->m_connection;
    net_nghttp2_service_t service = net_nghttp2_connection_service(connection);

    net_nghttp2_request_set_processor(request, NULL);
    assert(request->m_processor == NULL);

    if (request->m_response) {
        net_nghttp2_response_free(request->m_response);
        request->m_response = NULL;
    }
    ...
    while(!TAILQ_EMPTY(&request->m_headers)) {
        net_nghttp2_request_header_free(TAILQ_FIRST(&request->m_headers));
    }

    TAILQ_REMOVE(&connection->m_requests, request, m_next_for_connection);
    
    request->m_connection = (net_nghttp2_connection_t)service;
...
void net_nghttp2_request_real_free(net_nghttp2_request_t request) {
    net_nghttp2_service_t service = (net_nghttp2_service_t)request->m_connection;
...
void net_nghttp2_request_schedule_close_connection(net_nghttp2_request_t request) {
    net_nghttp2_request_set_state(request, net_nghttp2_request_state_complete);
    net_nghttp2_connection_schedule_close(request->m_connection);
}
...
net_nghttp2_request_t net_nghttp2_request_from_data(void * data) {
    return ((net_nghttp2_request_t)data) - 1;
}

void net_nghttp2_request_set_processor(net_nghttp2_request_t request, net_nghttp2_mount_point_t mp) {
...
void net_nghttp2_request_set_state(net_nghttp2_request_t request, net_nghttp2_request_state_t state) {
    ...
    if (net_endpoint_protocol_debug(net_endpoint_from_data(request->m_connection))) {
        ...
        CPE_INFO(
            ...
            net_nghttp2_request_state_str(request->m_state), 
            net_nghttp2_request_state_str(state));
    }
...
void net_nghttp2_request_print(write_stream_t ws, net_nghttp2_request_t request) {
    stream_printf(
        ws, "%s %d.%d", 
        net_nghttp2_request_method_str(request->m_method),
...
const char * net_nghttp2_request_dump(mem_buffer_t buffer, net_nghttp2_request_t request) {
    ...
    net_nghttp2_request_print((write_stream_t)&stream, request);
    stream_putc((write_stream_t)&stream, 0);
    return mem_buffer_make_continuous(buffer, 0);
}

void net_nghttp2_request_on_path(net_nghttp2_request_t request, const char* at, size_t length) {
    net_nghttp2_connection_t connection = request->m_connection;
    ...
    if (request->m_path == NULL) {
        ...
        net_nghttp2_connection_schedule_close(connection);
        return;
    }
    
    request->m_path_to_processor = request->m_path;

    net_nghttp2_mount_point_t mp = net_nghttp2_mount_point_find_by_path(service, &request->m_path_to_processor);
    if (mp == NULL) {
        CPE_ERROR(
    ...
    net_nghttp2_request_set_processor(request, mp);
...
void net_nghttp2_request_on_query_string(net_nghttp2_request_t request, const char* at, size_t length) {
    net_nghttp2_connection_t connection = request->m_connection;
    net_nghttp2_service_t service = net_nghttp2_connection_service(request->m_connection);
...
void net_nghttp2_request_on_uri(net_nghttp2_request_t request, const char* at, size_t length) {
    net_nghttp2_connection_t connection = request->m_connection;
    net_nghttp2_service_t service = net_nghttp2_connection_service(request->m_connection);
...
void net_nghttp2_request_on_fragment(net_nghttp2_request_t request, const char* at, size_t length) {
    net_nghttp2_connection_t connection = request->m_connection;
    net_nghttp2_service_t service = net_nghttp2_connection_service(request->m_connection);
...
void net_nghttp2_request_on_header_field(net_nghttp2_request_t request, const char* at, size_t length, int header_index) {
    net_nghttp2_connection_t connection = request->m_connection;
    net_nghttp2_service_t service = net_nghttp2_connection_service(request->m_connection);

    net_nghttp2_request_header_t header = net_nghttp2_request_header_create(request, (uint16_t)header_index, at, length);
    if (header == NULL) {
        CPE_INFO(
...
void net_nghttp2_request_on_header_value(net_nghttp2_request_t request, const char* at, size_t length, int header_index) {
    net_nghttp2_connection_t connection = request->m_connection;
    net_nghttp2_service_t service = net_nghttp2_connection_service(request->m_connection);

    net_nghttp2_request_header_t header = net_nghttp2_request_header_find_by_index(request, header_index);
    if (header == NULL) {
        CPE_ERROR(
    ...
    if (net_nghttp2_request_header_set_value(header, at, length) != 0) {
        CPE_INFO(
...
void net_nghttp2_request_on_body(net_nghttp2_request_t request, const char* at, size_t length) {
    net_nghttp2_connection_t connection = request->m_connection;
    net_nghttp2_service_t service = net_nghttp2_connection_service(request->m_connection);
...
void net_nghttp2_request_on_head_complete(net_nghttp2_request_t request) {
    net_nghttp2_connection_t connection = request->m_connection;
    net_nghttp2_service_t service = net_nghttp2_connection_service(request->m_connection);
    ...
    if (net_endpoint_protocol_debug(net_endpoint_from_data(connection)) >= 2) {
        CPE_INFO(
            ...
            net_endpoint_dump(net_nghttp2_service_tmp_buffer(service), net_endpoint_from_data(connection)), request->m_request_id);
    }

    if (request->m_state == net_nghttp2_request_state_complete) return;
    
    net_nghttp2_processor_t processor = request->m_processor;
    if (processor && processor->m_request_on_head_complete) {
        processor->m_request_on_head_complete(processor->m_ctx, request);
    }
}

void net_nghttp2_request_on_complete(net_nghttp2_request_t request) {
    net_nghttp2_connection_t connection = request->m_connection;
    net_nghttp2_service_t service = net_nghttp2_connection_service(request->m_connection);
    ...
    if (request->m_state != net_nghttp2_request_state_complete) {
        net_nghttp2_processor_t processor = request->m_processor;
    ...
    if (request->m_state == net_nghttp2_request_state_reading) {
        net_nghttp2_request_set_state(request, net_nghttp2_request_state_processing);
    }
    
    net_nghttp2_connection_check_remove_done_requests(connection);
}

const char * net_nghttp2_request_method_str(net_nghttp2_request_method_t method) {
    switch(method) {
    case net_nghttp2_request_method_unknown:
        return "unknown";
    case net_nghttp2_request_method_copy:
        return "copy";
    case net_nghttp2_request_method_delete:
        return "delete";
    case net_nghttp2_request_method_get:
        return "get";
    case net_nghttp2_request_method_head:
        return "head";
    case net_nghttp2_request_method_lock:
        return "lock";
    case net_nghttp2_request_method_mkcol:
        return "mkcol";
    case net_nghttp2_request_method_move:
        return "move";
    case net_nghttp2_request_method_options:
        return "options";
    case net_nghttp2_request_method_post:
        return "post";
    case net_nghttp2_request_method_propfind:
        return "propfind";
    case net_nghttp2_request_method_proppatch:
        return "proppatch";
    case net_nghttp2_request_method_put:
        return "put";
    case net_nghttp2_request_method_trace:
        return "trace";
    case net_nghttp2_request_method_unlock:
        return "unlock";
    }
}
...
const char * net_nghttp2_request_state_str(net_nghttp2_request_state_t state) {
    switch(state) {
    case net_nghttp2_request_state_reading:
        return "reading";
    case net_nghttp2_request_state_processing:
        return "processing";
    case net_nghttp2_request_state_complete:
        return "complete";
    }
}
...
const char * net_nghttp2_request_transfer_encoding_str(net_nghttp2_request_transfer_encoding_t transfer_encoding) {
    ...
    case net_nghttp2_request_transfer_encoding_unknown:
        return "unknown";
    case net_nghttp2_request_transfer_encoding_identity:
        return "identity";
    case net_nghttp2_request_transfer_encoding_chunked:
        return "chunked";
    }
}
