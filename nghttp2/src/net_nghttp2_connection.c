...
static net_nghttp2_request_t net_nghttp2_connection_new_request(void *data);
static void net_nghttp2_connection_timeout_cb(net_timer_t timer, void * ctx);
static void net_nghttp2_connection_close_cb(net_timer_t timer, void * ctx);

int net_nghttp2_connection_init(net_endpoint_t base_endpoint) {
    net_nghttp2_service_t service = net_protocol_data(net_endpoint_protocol(base_endpoint));
    net_nghttp2_connection_t connection = net_endpoint_data(base_endpoint);

    connection->m_timer_timeout = net_timer_auto_create(net_endpoint_schedule(base_endpoint), net_nghttp2_connection_timeout_cb, connection);
    if (connection->m_timer_timeout == NULL) {
        CPE_ERROR(
            service->m_em, "nghttp2: %s: create timeout timer fail", 
            net_endpoint_dump(net_nghttp2_service_tmp_buffer(service), base_endpoint));
        return -1;
    }

    connection->m_timer_close = net_timer_auto_create(net_endpoint_schedule(base_endpoint), net_nghttp2_connection_close_cb, connection);
    if (connection->m_timer_close == NULL) {
        CPE_ERROR(
            service->m_em, "nghttp2: %s: create close timer fail", 
            net_endpoint_dump(net_nghttp2_service_tmp_buffer(service), base_endpoint));
        net_timer_free(connection->m_timer_timeout);
        return -1;
    }
    
    /* net_nghttp2_request_parser_init(&connection->parser); */
    /* connection->parser.data = connection; */
    /* connection->parser.new_request = net_nghttp2_connection_new_request; */

    TAILQ_INIT(&connection->m_requests);
    
    net_nghttp2_connection_timeout_reset(connection);

    return 0;
}

void net_nghttp2_connection_fini(net_endpoint_t base_endpoint) {
    //net_nghttp2_service_t service = net_protocol_data(net_endpoint_protocol(base_endpoint));
    net_nghttp2_connection_t connection = net_endpoint_data(base_endpoint);

    while(!TAILQ_EMPTY(&connection->m_requests)) {
        net_nghttp2_request_free(TAILQ_FIRST(&connection->m_requests));
    }
...
int net_nghttp2_connection_input(net_endpoint_t base_endpoint) {
    net_nghttp2_service_t service = net_protocol_data(net_endpoint_protocol(base_endpoint));
    net_nghttp2_connection_t connection = net_endpoint_data(base_endpoint);
    ...
    while(net_endpoint_state(base_endpoint) == net_endpoint_state_established) {
        ...
        //net_nghttp2_request_parser_execute(&connection->parser, data, data_size);
        ...
        /* if (net_nghttp2_request_parser_has_error(&connection->parser)) { */
        /*     CPE_ERROR( */
...
net_nghttp2_service_t net_nghttp2_connection_service(net_nghttp2_connection_t connection) {
...
static net_nghttp2_request_t net_nghttp2_connection_new_request(void *data) {
    net_nghttp2_connection_t connection = data;
    return net_nghttp2_request_create(connection);;
}

static void net_nghttp2_connection_timeout_cb(net_timer_t timer, void * ctx) {
    net_nghttp2_connection_t connection = ctx;
    net_endpoint_t base_endpoint = net_endpoint_from_data(connection);
    net_nghttp2_service_t service = net_protocol_data(net_endpoint_protocol(base_endpoint));

    if (net_endpoint_protocol_debug(base_endpoint)) {
        CPE_INFO(
            service->m_em, "nghttp2: %s: timeout",
            net_endpoint_dump(net_nghttp2_service_tmp_buffer(service), base_endpoint));
    }
...
static void net_nghttp2_connection_close_cb(net_timer_t timer, void * ctx) {
    ...
    if (net_endpoint_protocol_debug(base_endpoint)) {
        CPE_INFO(
            service->m_em, "nghttp2: %s: do close",
            net_endpoint_dump(net_nghttp2_service_tmp_buffer(service), base_endpoint));
    }
...
void net_nghttp2_connection_check_remove_done_requests(net_nghttp2_connection_t connection) {
    net_nghttp2_request_t request = TAILQ_FIRST(&connection->m_requests);
