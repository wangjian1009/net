...
static void net_http2_stream_pair_expect_read(void **state) {
    net_http2_testenv_t env = *state;
    net_http2_endpoint_t ep = net_http2_testenv_stream_cli_ep_create(env, "1.2.3.4:5678");
    net_endpoint_t ep_base = net_http2_endpoint_base_endpoint(ep);
    /* //net_endpoint_t underline = net_http2_endpoint_underline(ep_base); */
...
static void net_http2_pair_connect_success(void **state) {
    net_http2_testenv_t env = *state;
    net_http2_endpoint_t ep = net_http2_testenv_stream_cli_ep_create(env, "1.2.3.4:5678");
    net_endpoint_t ep_base = net_http2_endpoint_base_endpoint(ep);
    /* net_endpoint_t underline = net_http2_endpoint_underline(ep_base); */
...
static void net_http2_pair_connect_success_delay(void **state) {
    net_http2_testenv_t env = *state;
    net_http2_endpoint_t ep = net_http2_testenv_stream_cli_ep_create(env, "1.2.3.4:5678");
    net_endpoint_t ep_base = net_http2_endpoint_base_endpoint(ep);
    /* net_endpoint_t underline = net_http2_endpoint_underline(ep_base); */
...
int net_http2_cli_basic_tests() {
	const struct CMUnitTest ws_basic_tests[] = {
		//cmocka_unit_test_setup_teardown(net_http2_pair_expect_read, setup, teardown),
		cmocka_unit_test_setup_teardown(net_http2_pair_connect_success, setup, teardown),
		cmocka_unit_test_setup_teardown(net_http2_pair_connect_success_delay, setup, teardown),
